...
Refer to the section [Event message](/AbeewayRefGuide/uplink-messages/event/readme.md) for more details.
:::
...
In the case where the GPS module didn't succeed, a GPS timeout message is sent instead of a GPS position message. The timeout cause is given in the payload. (See [GPS timeout payload](/AbeewayRefGuide/uplink-messages/position/readme.md) for more details about this message)
