...
With this technology, the device sends the data given by the GPS module before the *agps_timeout* delay expiry and the positions calculation are done by the ABEEWAY server.

For power consumption purpose, a predictive algorithm has been put in place: The GPS is stopped as soon as possible if it has no chance to make a fix or to acquire enough data for LPGPS calculation (e.g. indoor location without a clear sky view).
...
*ble_beacon_cnt* = 0x04: The maximum number of beacons sent in LoRaWAN^TM^ position messages is 4
