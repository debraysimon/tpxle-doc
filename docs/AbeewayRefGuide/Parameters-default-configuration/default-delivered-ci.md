...
The tracker's mode must be set in motion, for positions to be sent using **GPS** technology every **5 minutes** using [**dual random transmit** strategy](../Parameters-default-configuration/firmware-parameters.md#lora-parameters). The **heartbeat** is sent every **10 minutes**.
