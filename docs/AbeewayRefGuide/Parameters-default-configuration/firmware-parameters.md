...
| motion_nb_pos   | 0x08 | none   | 0 - 20        | Number of positions to report during motion start/end events. </br> **Note: This parameter has been updated in FW2.3. In motion tracking mode: additional positions are done when the device goes to static (after motion_duration period). If set to 0, no positions are sent in motion start/end mode.**  |
